...
public class CardOptionWindow : MonoBehaviour
{
    ...
    public PlayerFieldManager m_myPlayerFieldManager = null;
    public FieldCardManager m_myFieldCardManager = null;

    private Image target = null;
    public PlayerFieldManager targetplayerFieldManager = null;
    public FieldCardManager targetFieldCardManager = null;
    ...
    private void SetButtonToBs()
    {
        ...
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.F1, false, "ドロー&コア&リフレッシュステップ", () => {
            ...
            m_myPlayerFieldManager.OnClickPlusButton();
    ...
    private void SetButtonToOption()
    {
        ...
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, "ランダムに手札からX枚トラッシュに送る", () => {
            for (int index = 0; index < numButtonTextNum; index++)
            {
                ...
                targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.HAND, OPTION_TYPE.TRASH, handObject.GetComponent<Image>(), list[0], list[1]);
            }
            CloseOnSound();
        }));
        ...
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, "トークンをフィールドに送る", () => {
            m_myPlayerFieldManager.CreateCard(target.name);
            CloseOnSound();
        }));
        ...
        AddInputActionList(targetplayerFieldManager, targetFieldCardManager, KeyCodeManager.InputType.GET_DOWN, KeyCode.T, "トークン一覧の一種類目をフィールドに送る", () =>
        {
            ...
            if (cardList.Count > 0)
            {
                m_myPlayerFieldManager.CreateCard(cardList[0].ToString());
            }
        });
    ...
    private void SetButtonToDeck()
    {
        ...
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, "デッキからX枚を裏向きでフィールドに送る", () => {
            ...
            {
                var card = targetFieldCardManager.RemoveCardDetail(OPTION_TYPE.DECK, cardDetail.tag, cardDetail.cardId)[0];
                Image cardImage = m_myPlayerFieldManager.CreateCard(cardDetail.ToString(), false);
        ...
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, "デッキの下に送る", () => {
            string[] list = target.name.Split('^');
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.DECK, OPTION_TYPE.DECK, false, list[0], list[1]);
            CloseOnSound();
        }));
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, "デッキから手札に送る", () => {
            string[] list = target.name.Split('^');
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.DECK, OPTION_TYPE.HAND, list[0], list[1]);
            CloseOnSound();
        }));
        ...
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, "デッキから" + atHandStr + "の上に送る", () => {
            string[] list = target.name.Split('^');
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.DECK, OPTION_TYPE.AT_HAND, true, list[0], list[1]);
            CloseOnSound();
        }));
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, "デッキから" + atHandStr + "の上に送る", () => {
            string[] list = target.name.Split('^');
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.DECK, OPTION_TYPE.AT_HAND, false, list[0], list[1]);
            CloseOnSound();
        }));
        ...
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, "デッキから" + damageStr + "の上に送る", () => {
            string[] list = target.name.Split('^');
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.DECK, OPTION_TYPE.DAMAGE, true, list[0], list[1]);
            CloseOnSound();
        }));
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, "デッキから" + subStr + "の上に送る", () => {
            string[] list = target.name.Split('^');
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.DECK, OPTION_TYPE.SUB, true, list[0], list[1]);
            CloseOnSound();
        }));
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, "デッキから" + damageStr + "の下に送る", () => {
            string[] list = target.name.Split('^');
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.DECK, OPTION_TYPE.DAMAGE, false, list[0], list[1]);
            CloseOnSound();
        }));
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, "デッキから" + subStr + "の下に送る", () => {
            string[] list = target.name.Split('^');
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.DECK, OPTION_TYPE.SUB, false, list[0], list[1]);
            CloseOnSound();
        }));

        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, "デッキからトラッシュに送る", () => {
            string[] list = target.name.Split('^');
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.DECK, OPTION_TYPE.TRASH, list[0], list[1]);
            CloseOnSound();
        }));
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, "デッキから除外一覧に送る", () => {
            string[] list = target.name.Split('^');
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.DECK, OPTION_TYPE.EXCLUSION, list[0], list[1]);
            CloseOnSound();
        }));
        ...
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, "デッキから契約カードを手札に送る", () => {
            targetFieldCardManager.AddHandFromDeckToContract();
            int hand = (int)PhotonNetwork.CurrentRoom.CustomProperties["Hand"];
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.DECK, OPTION_TYPE.HAND, true, (hand - 1));
            CloseOnSound();
        }));
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, "デッキから契約カードを手札に送らない", () => {
            int hand = (int)PhotonNetwork.CurrentRoom.CustomProperties["Hand"];
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.DECK, OPTION_TYPE.HAND, true, hand);
            CloseOnSound();
        }));
    ...
    public void SetButtonToHand()
    {
        ...
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, "手札からデッキの上に送る", () => {
            string[] list = target.name.Split('^');
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.HAND, OPTION_TYPE.DECK, true, target, list[0], list[1]);
            CloseOnSound();
        }));
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, "手札からデッキの下に送る", () => {
            string[] list = target.name.Split('^');
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.HAND, OPTION_TYPE.DECK, false, target, list[0], list[1]);
            CloseOnSound();
        }));
        ...
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, "手札から" + atHandStr + "の上に送る", () => {
            string[] list = target.name.Split('^');
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.HAND, OPTION_TYPE.AT_HAND, true, target, list[0], list[1]);
            CloseOnSound();
        }));
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, "手札から" + atHandStr + "の下に送る", () => {
            string[] list = target.name.Split('^');
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.HAND, OPTION_TYPE.AT_HAND, false, target, list[0], list[1]);
            CloseOnSound();
        }));
        ...
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, "手札から" + damageStr + "の上に送る", () => {
            string[] list = target.name.Split('^');
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.HAND, OPTION_TYPE.DAMAGE, true, target, list[0], list[1]);
            CloseOnSound();
        }));
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, "手札から" + subStr + "の上に送る", () => {
            string[] list = target.name.Split('^');
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.HAND, OPTION_TYPE.SUB, true, target, list[0], list[1]);
            CloseOnSound();
        }));
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, "手札から" + damageStr + "の下に送る", () => {
            string[] list = target.name.Split('^');
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.HAND, OPTION_TYPE.DAMAGE, false, target, list[0], list[1]);
            CloseOnSound();
        }));
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, "手札から" + subStr + "の下に送る", () => {
            string[] list = target.name.Split('^');
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.HAND, OPTION_TYPE.SUB, false, target, list[0], list[1]);
            CloseOnSound();
        }));

        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, "手札からトラッシュに送る", () => {
            string[] list = target.name.Split('^');
            var card = targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.HAND, OPTION_TYPE.TRASH, target, list[0], list[1]);
            CardListWindow.Instance().Open(targetplayerFieldManager, targetFieldCardManager, CardOptionWindow.OPTION_TYPE.CARD_LIST, new List<DeckManager.CardDetail>() { card });
            CloseOnSound();
        }));
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, "手札から除外一覧に送る", () => {
            string[] list = target.name.Split('^');
            var card = targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.HAND, OPTION_TYPE.EXCLUSION, target, list[0], list[1]);
            CardListWindow.Instance().Open(targetplayerFieldManager, targetFieldCardManager, CardOptionWindow.OPTION_TYPE.CARD_LIST, new List<DeckManager.CardDetail>() { card });
            CloseOnSound();
        }));
    }
    ...
    public void SetButtonToAtHand()
    {
        ...
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, atHandStr + "からフィールドに送る", () => {
            string[] list = target.name.Split('^');
            targetFieldCardManager.RemoveCardImage(OPTION_TYPE.AT_HAND, target);
            targetplayerFieldManager.CreateCard(target.name);
            CloseOnSound();
        }));
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, atHandStr + "からデッキの上に送る", () => {
            string[] list = target.name.Split('^');
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.AT_HAND, OPTION_TYPE.DECK, true, target, list[0], list[1]);
            CloseOnSound();
        }));
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, atHandStr + "からデッキの下に送る", () => {
            string[] list = target.name.Split('^');
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.AT_HAND, OPTION_TYPE.DECK, false, target, list[0], list[1]);
            CloseOnSound();
        }));
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, atHandStr + "から手札に送る", () => {
            string[] list = target.name.Split('^');
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.AT_HAND, OPTION_TYPE.HAND, target, list[0], list[1]);
            CloseOnSound();
        }));
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, atHandStr + "からトラッシュに送る", () => {
            string[] list = target.name.Split('^');
            var card = targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.AT_HAND, OPTION_TYPE.TRASH, target, list[0], list[1]);
            CardListWindow.Instance().Open(targetplayerFieldManager, targetFieldCardManager, CardOptionWindow.OPTION_TYPE.CARD_LIST, new List<DeckManager.CardDetail>() { card });
            CloseOnSound();
        }));
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, atHandStr + "から除外一覧に送る", () => {
            string[] list = target.name.Split('^');
            var card = targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.AT_HAND, OPTION_TYPE.EXCLUSION, target, list[0], list[1]);
            CardListWindow.Instance().Open(targetplayerFieldManager, targetFieldCardManager, CardOptionWindow.OPTION_TYPE.CARD_LIST, new List<DeckManager.CardDetail>() { card });
            CloseOnSound();
        }));
    }
    ...
    public void SetButtonToField()
    {
        ...
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, "フィールドからデッキの上に送る", () => {
            ...
            if (touchManager != null)
            {
                ...
                {
                    targetFieldCardManager.AddDstFromSrc(CardOptionWindow.OPTION_TYPE.FIELD, innerListFromType, true, null, cardDetail.tag, cardDetail.cardId);
                }
            }
            string[] list = target.name.Split('^');
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.FIELD, OPTION_TYPE.DECK, true, target, list[0], list[1]);
            CloseOnSound();
        }));
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, "フィールドからデッキの下に送る", () => {
            ...
            string[] list = target.name.Split('^');
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.FIELD, OPTION_TYPE.DECK, false, target, list[0], list[1]);
            CloseOnSound();
        }));
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, "フィールドから手札に送る", () => {
            ...
            string[] list = target.name.Split('^');
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.FIELD, OPTION_TYPE.HAND, target, list[0], list[1]);
            CloseOnSound();
        }));
        ...
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, "フィールドから" + atHandStr + "の上に送る", () => {
            ...
            string[] list = target.name.Split('^');
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.FIELD, OPTION_TYPE.AT_HAND, target, list[0], list[1]);
            CloseOnSound();
        }));
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, "フィールドから" + atHandStr + "の下に送る", () => {
            ...
            if (touchManager != null)
            {
                foreach (var cardDetail in touchManager.m_innerCardDetailList)
                {
                    targetFieldCardManager.AddDstFromSrc(CardOptionWindow.OPTION_TYPE.FIELD, innerListFromType, false, null, cardDetail.tag, cardDetail.cardId);
                }
            }
            string[] list = target.name.Split('^');
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.FIELD, OPTION_TYPE.AT_HAND, target, list[0], list[1]);
        ...
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, "フィールドから" + damageStr + "の上に送る", () => {
            ...
            string[] list = target.name.Split('^');
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.FIELD, OPTION_TYPE.DAMAGE, true, target, list[0], list[1]);
            CloseOnSound();
        }));
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, "フィールドから" + subStr + "の上に送る", () => {
            ...
            string[] list = target.name.Split('^');
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.FIELD, OPTION_TYPE.SUB, true, target, list[0], list[1]);
            CloseOnSound();
        }));
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, "フィールドから" + damageStr + "の下に送る", () => {
            ...
            string[] list = target.name.Split('^');
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.FIELD, OPTION_TYPE.DAMAGE, false, target, list[0], list[1]);
            CloseOnSound();
        }));
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, "フィールドから" + subStr + "の下に送る", () => {
            ...
            string[] list = target.name.Split('^');
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.FIELD, OPTION_TYPE.SUB, false, target, list[0], list[1]);
            CloseOnSound();
        }));
        ...
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, "フィールドからトラッシュに送る", () => {
            ...
            string[] list = target.name.Split('^');
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.FIELD, OPTION_TYPE.TRASH, target, list[0], list[1]);
            CloseOnSound();
        }));
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, "フィールドから除外一覧に送る", () => {
            ...
            string[] list = target.name.Split('^');
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.FIELD, OPTION_TYPE.EXCLUSION, target, list[0], list[1]);
            CloseOnSound();
        }));
    }
    ...
    public void SetButtonToExclusion()
    {
        ...
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, "相手の除外一覧を確認する", () => {
            ...
            foreach (var playFieldObj in playFieldList)
            {
                ...
                if (fieldCardManager == null || fieldCardManager == targetFieldCardManager)
                {
                    continue;
                }
            ...
            if (playField != null)
            {
                ...
                CardListWindow.Instance().Open(targetplayerFieldManager, targetFieldCardManager, CardOptionWindow.OPTION_TYPE.EXCLUSION, openCardList, false);
            }
            Close();
        }));
        ...
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, "除外一覧からデッキの上に送る", () => {
            string[] list = target.name.Split('^');
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.EXCLUSION, OPTION_TYPE.DECK, true, list[0], list[1]);
        ...
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, "除外一覧からデッキの下に送る", () => {
            string[] list = target.name.Split('^');
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.EXCLUSION, OPTION_TYPE.DECK, false, list[0], list[1]);
            OpenCardDetailListToMine(OPTION_TYPE.EXCLUSION);
        }));
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, "除外一覧から手札に送る", () => {
            string[] list = target.name.Split('^');
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.EXCLUSION, OPTION_TYPE.HAND, list[0], list[1]);
            OpenCardDetailListToMine(OPTION_TYPE.EXCLUSION);
        }));
        ...
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, "除外一覧から" + atHandStr + "の上に送る", () => {
            string[] list = target.name.Split('^');
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.EXCLUSION, OPTION_TYPE.AT_HAND, true, list[0], list[1]);
            OpenCardDetailListToMine(OPTION_TYPE.EXCLUSION);
        }));
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, "除外一覧から" + atHandStr + "の下に送る", () => {
            string[] list = target.name.Split('^');
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.EXCLUSION, OPTION_TYPE.AT_HAND, false, list[0], list[1]);
            OpenCardDetailListToMine(OPTION_TYPE.EXCLUSION);
        }));

        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, "除外一覧から" + damageStr + "の上に送る", () => {
            string[] list = target.name.Split('^');
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.EXCLUSION, OPTION_TYPE.DAMAGE, true, list[0], list[1]);
            OpenCardDetailListToMine(OPTION_TYPE.EXCLUSION);
        }));
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, "除外一覧から" + subStr + "の上に送る", () => {
            string[] list = target.name.Split('^');
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.EXCLUSION, OPTION_TYPE.SUB, true, list[0], list[1]);
            OpenCardDetailListToMine(OPTION_TYPE.EXCLUSION);
        }));
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, "除外一覧から" + damageStr + "の下に送る", () => {
            string[] list = target.name.Split('^');
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.EXCLUSION, OPTION_TYPE.DAMAGE, false, list[0], list[1]);
            OpenCardDetailListToMine(OPTION_TYPE.EXCLUSION);
        }));
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, "除外一覧から" + subStr + "の下に送る", () => {
            string[] list = target.name.Split('^');
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.EXCLUSION, OPTION_TYPE.SUB, false, list[0], list[1]);
            OpenCardDetailListToMine(OPTION_TYPE.EXCLUSION);
        }));

        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, "除外一覧からトラッシュに送る", () => {
            string[] list = target.name.Split('^');
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.EXCLUSION, OPTION_TYPE.TRASH, list[0], list[1]);
            OpenCardDetailListToMine(OPTION_TYPE.EXCLUSION);
        }));
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, "除外一覧からフィールドに送る", () => {
            string[] list = target.name.Split('^');
            var card = targetFieldCardManager.RemoveCardDetail(OPTION_TYPE.EXCLUSION, list[0], list[1])[0];
            targetplayerFieldManager.CreateCard(target.name);
            OpenCardDetailListToMine(OPTION_TYPE.EXCLUSION);
        }));
    }
    ...
    public void SetButtonToTrash()
    {
        ...
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, "相手のトラッシュを確認する", () => {
            ...
            if (playField != null)
            {
                ...
                CardListWindow.Instance().Open(targetplayerFieldManager, targetFieldCardManager, CardOptionWindow.OPTION_TYPE.TRASH, openCardList, false);
            }
            Close();
        }));
        ...
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, "トラッシュからデッキの上に送る", () => {
            string[] list = target.name.Split('^');
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.TRASH, OPTION_TYPE.DECK, true, list[0], list[1]);
        ...
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, "トラッシュからデッキの下に送る", () => {
            string[] list = target.name.Split('^');
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.TRASH, OPTION_TYPE.DECK, false, list[0], list[1]);
            OpenCardDetailListToMine(OPTION_TYPE.TRASH);
        }));
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, "トラッシュから手札に送る", () => {
            string[] list = target.name.Split('^');
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.TRASH, OPTION_TYPE.HAND, list[0], list[1]);
            OpenCardDetailListToMine(OPTION_TYPE.TRASH);
        }));
        ...
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, "トラッシュから" + atHandStr + "の下に送る", () => {
            string[] list = target.name.Split('^');
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.TRASH, OPTION_TYPE.AT_HAND, false, list[0], list[1]);
            OpenCardDetailListToMine(OPTION_TYPE.TRASH);
        }));

        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, "トラッシュから" + damageStr + "の上に送る", () => {
            string[] list = target.name.Split('^');
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.TRASH, OPTION_TYPE.DAMAGE, true, list[0], list[1]);
            OpenCardDetailListToMine(OPTION_TYPE.TRASH);
        }));
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, "トラッシュから" + subStr + "の上に送る", () => {
            string[] list = target.name.Split('^');
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.TRASH, OPTION_TYPE.SUB, true, list[0], list[1]);
            OpenCardDetailListToMine(OPTION_TYPE.TRASH);
        }));
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, "トラッシュから" + damageStr + "の下に送る", () => {
            string[] list = target.name.Split('^');
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.TRASH, OPTION_TYPE.DAMAGE, false, list[0], list[1]);
            OpenCardDetailListToMine(OPTION_TYPE.TRASH);
        }));
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, "トラッシュから" + subStr + "の下に送る", () => {
            string[] list = target.name.Split('^');
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.TRASH, OPTION_TYPE.SUB, false, list[0], list[1]);
            OpenCardDetailListToMine(OPTION_TYPE.TRASH);
        }));
    ...
    private void SetButtonToDamage()
    {
        ...
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, damageStr + "をシャッフルする", () => {
            targetFieldCardManager.ShuffleCardDetailList(OPTION_TYPE.DAMAGE);
            Close();
            AudioSourceManager.Instance().PlayOneShot(1);
        }));
        ...
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, damageStr + "からフィールドに出す", () => {
            string[] list = target.name.Split('^');
            var card = targetFieldCardManager.RemoveCardDetail(OPTION_TYPE.DAMAGE, list[0], list[1])[0];
            targetplayerFieldManager.CreateCard(target.name);
            CloseOnSound();
        }));
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, damageStr + "の上に送る", () => {
            string[] list = target.name.Split('^');
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.DAMAGE, OPTION_TYPE.DAMAGE, true, list[0], list[1]);
            CloseOnSound();
        }));
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, damageStr + "の下に送る", () => {
            string[] list = target.name.Split('^');
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.DAMAGE, OPTION_TYPE.DAMAGE, false, list[0], list[1]);
            CloseOnSound();
        }));
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, damageStr + "から手札に送る", () => {
            string[] list = target.name.Split('^');
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.DAMAGE, OPTION_TYPE.HAND, list[0], list[1]);
            CloseOnSound();
        }));
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, damageStr + "からトラッシュに送る", () => {
            ...
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.DAMAGE, OPTION_TYPE.TRASH, list[0], list[1]);
            CloseOnSound();
        }));
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, damageStr + "から除外一覧に送る", () => {
            string[] list = target.name.Split('^');
            List<bool> isTrashList = new List<bool>() { true, false };
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.DAMAGE, OPTION_TYPE.EXCLUSION, list[0], list[1]);
            CloseOnSound();
        }));
    }
    ...
    private void SetButtonToSub()
    {
        ...
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, subStr + "からフィールドに送る", () => {
            string[] list = target.name.Split('^');
            var card = targetFieldCardManager.RemoveCardDetail(OPTION_TYPE.SUB, list[0], list[1])[0];
            targetplayerFieldManager.CreateCard(target.name);
            CloseOnSound();
        }));
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, subStr + "の上に送る", () => {
            string[] list = target.name.Split('^');
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.SUB, OPTION_TYPE.SUB, true, list[0], list[1]);
            CloseOnSound();
        }));
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, subStr + "の下に送る", () => {
            string[] list = target.name.Split('^');
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.SUB, OPTION_TYPE.SUB, false, list[0], list[1]);
            CloseOnSound();
        }));
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, subStr + "から手札に送る", () => {
            string[] list = target.name.Split('^');
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.SUB, OPTION_TYPE.HAND, list[0], list[1]);
            CloseOnSound();
        }));
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, subStr + "からトラッシュに送る", () => {
            string[] list = target.name.Split('^');
            List<bool> isTrashList = new List<bool>() { true, false };
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.SUB, OPTION_TYPE.TRASH, list[0], list[1]);
            CloseOnSound();
        }));
        m_optionButtonList.Add(new OptionButton(optionType, detailOptionType, KeyCode.None, false, subStr + "から除外一覧に送る", () => {
            string[] list = target.name.Split('^');
            List<bool> isTrashList = new List<bool>() { true, false };
            targetFieldCardManager.AddDstFromSrc(OPTION_TYPE.SUB, OPTION_TYPE.EXCLUSION, list[0], list[1]);
            CloseOnSound();
        }));
    ...
    public void AddLogList(string str)
    {
        ...
        if (targetplayerFieldManager != null)
        {
            targetplayerFieldManager.AddLogList(str);
        }
    }
    ...
    private void OpenCardDetailListToMine(OPTION_TYPE option, List<DeckManager.CardDetail> cardList = null)
    {
        ...
        if (cardList == null)
        {
            cardList = targetFieldCardManager.GetCardDetailList(option);
        }
        ...
        CardListWindow.Instance().Open(targetplayerFieldManager, targetFieldCardManager, option, openCardList);
        CloseOnSound();
    }

    public void Open(PlayerFieldManager playerFieldManager, FieldCardManager fieldCardManager, Image card, OPTION_TYPE optionType, OPTION_TYPE detailOptionType = OPTION_TYPE.NONE, Action<bool> callback = null)
    {
        target = card;
        TouchManager touchManager = null;
        HandCard handCard = null;
        if (playerFieldManager)
        {
            targetplayerFieldManager = playerFieldManager;
        }
        ...
        if (target != null)
        {
            ...
            if (touchManager != null)
            {
                targetplayerFieldManager = touchManager.m_playerFieldManager;
            }
            handCard = target.GetComponent<HandCard>();
            if (handCard != null)
            {
                targetplayerFieldManager = handCard.m_playerFieldManager;
            }
        }

        if (fieldCardManager != null)
        {
            targetFieldCardManager = fieldCardManager;
        }
        else
        {
            touchManager = target.GetComponent<TouchManager>();
        ...
        Open(targetplayerFieldManager, targetFieldCardManager, optionType, detailOptionType, callback);
    }

    public void Open(PlayerFieldManager playerFieldManager, FieldCardManager fieldCardManager, OPTION_TYPE optionType, OPTION_TYPE detailOptionType = OPTION_TYPE.NONE, Action<bool> callback = null)
    {
        targetplayerFieldManager = playerFieldManager;
        targetFieldCardManager = fieldCardManager;
        bool isActiveAtHand = targetFieldCardManager.IsActiveAtHand();
    ...
    {
        target = null;
        targetplayerFieldManager = null;
        targetFieldCardManager = null;
